...
<html lang="es">
	...
	<body background="images\webb.png">
		...
		<main>
				...
				try{	
					$busqueda=$_GET['busquedatexto'];
					$busquedaBien="%".$busqueda."%";
				$query="SELECT NOMBRE,DESCRIPCION,PRECIOBASE,IVA,OID_PRODUCTO FROM PRODUCTOS WHERE NOMBRE LIKE  '%$busqueda%' ";
	...
	$_SESSION["paginacion"] = $paginacion;
		...
		header("Location: error_conexionBD.php");
			...
			<?php }	else { ?>
						...
						<a href="busquedaTexto.php? PAG_NUM=<?php echo $pagina; ?>&PAG_TAM=<?php echo $pag_tam; ?>"><?php echo $pagina; ?></a>

			<?php } ?>

		</div>
		
		<form method="get" action="busquedaTexto.php?" class="formularioCambiarPagina">
			<input id="busquedatext" name="busquedatexto" class="barraBusqueda" type="hidden" value="<?php echo $busqueda?>"   />
